...
*`p5js/skyline.html?lon=-1.5&lat=55.0&radii=5,10,15` — three rings instead of one, each dashed ring its own eye-level baseline, each solid line the real skyline at that radius, radius labels stacked to the north-west. Fainter near the centre, more solid further out, purely so the outermost — usually the one with most going on — reads as the primary line; the ground genuinely falls away to the east (all three dip there, across the estuary) and rises into the hills to the south-west.*
